...
object Interaction2 {
  ...
  def availableLayers: Seq[Layer] = {
    val temperature = Layer(LayerName.Temperatures,
      Seq(
      ...
      1975 to 2015)
    val deviation = Layer(LayerName.Deviations,
      Seq(
        (7.0, Color(0, 0, 0)),
        (4.0, Color(255, 0, 0)),
        (2.0, Color(255, 255, 0)),
  ...
  def layerUrlPattern(selectedLayer: Signal[Layer], selectedYear: Signal[Year]): Signal[String] = {
    Signal(s"images/${selectedLayer().layerName.id}/${selectedYear()}/{z}/{x}-{y}.png")
  }
